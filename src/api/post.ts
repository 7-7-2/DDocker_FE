...
export const registerPost = async (postForm: PostForm) => {
  const res = await authInstance.post(`${API}/posts/register`, postForm);
  return res.data;
};

// 3. 포스트 삭제(+JWT 인증)
export const deletePost = async (postId: string) => {
  const res = await authInstance.delete(`${API}posts/${postId}`);
  return res.data;
};
...
export const updatePost = async (
  ...
  const res = await authInstance.patch(`${API}posts/${postId}`, postForm);
  return res.data;
};
...
export const replyComment = async (commentId: number) => {
  const res = await authInstance.post(`${API}posts/${commentId}/reply`);
  return res.data;
};
...
export const getComments = async (postId: string) => {
  const res = await baseInstance.get(`${API}posts/${postId}/comments`);
  return res.data;
};

// 10. 댓글 하단 더보기 클릭시 답글목록 조회
export const getReply = async (commentId: number) => {
  const res = await baseInstance.get(`${API}posts/${commentId}/reply`);
  return res.data;
};
...
export const getSocialCounts = async (postId: string) => {
  const res = await baseInstance.get(`${API}posts/${postId}/counts`);
  return res.data;
};

//13. 메인 페이지 인기 브랜드 순위 랭킹 조회
export const getRanking = async () => {
  const res = await authInstance.get(`${API}popular`);
  return res.data;
};
